...
from Recommender.config.db import MODEL_DIR
from Recommender.core.constants import MODEL_SUFFIX, ENCODER_SUFFIX
...
def load_model_files(user_id: str) -> dict:
    version = get_model_registry(user_id)
    model_path = model_file(user_id, MODEL_SUFFIX, version)
    encoder_path = model_file(user_id, ENCODER_SUFFIX, version)

    try:
        model = joblib.load(model_path)
        encoders = joblib.load(encoder_path)
    except FileNotFoundError as e:
        raise FileNotFoundError(f"[MODEL] Missing file: {str(e)}")
    except Exception as e:
        raise RuntimeError(f"[MODEL] Failed to load model or encoder: {str(e)}")

    logger.debug(f"[MODEL] Loaded recommender model v{version} for user {user_id}")
    return {
        MODEL_SUFFIX: model,
        ENCODER_SUFFIX: encoders
    }
...
def update_model_registry(user_id: str, version: int, task_count: int, is_baseline: bool) -> None:
    ...
    if REGISTRY_PATH.exists():
        try:
            ...
            logger.warning(f"[REGISTRY] Failed to read existing registry: {e}")
